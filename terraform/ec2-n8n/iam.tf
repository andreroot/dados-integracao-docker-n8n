...
resource "aws_iam_role" "n8n-integ_ec2_execution_role" {
  name = "n8n-integ-ec2-role"
...
data "aws_iam_policy" "n8n-integ_management_group_policy" {
  for_each = toset(["AmazonEC2ContainerRegistryReadOnly", "AmazonECSTaskExecutionRolePolicy"])
  name     = each.value
}

# politica para log stream no cloudwatch
resource "aws_iam_role_policy_attachment" "n8n-integ_ec2_execution_role_policy" {
  role       = aws_iam_role.n8n-integ_ec2_execution_role.name
  for_each = data.aws_iam_policy.n8n-integ_management_group_policy
